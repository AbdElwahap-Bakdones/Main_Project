...
class QueryFields(object):
    ...
    def __checkPermission(user: object, operation: str) -> bool:
        return permission.checkPermission(operation, user)
    ...
    def is_valide(info: ResolveInfo, user: object, operation: str) -> bool:
        if QueryFields.__checkPermission(user, operation):
    ...
    def __no_permission(user: object):
        ...
        QueryFields.return_dict[user]['status'] = status_code.HTTP_401_UNAUTHORIZED
        QueryFields.return_dict[user]['message'] = 'you dont have permission'
        QueryFields.return_dict[user]['data'] = []
        return []
