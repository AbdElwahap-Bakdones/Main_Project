...
class SerchPlayer  (ObjectType, QueryFields):
    ...
    data = relay.ConnectionField(relays.PlayerConnection, player_Name=graphene.String(), player_Email=graphene.String())
...
def GetFriendByName(name: str,  user: models.User) -> models.Friend.objects:
    if name.count(' ') > 1:
        ...
        friend = models.Friend.objects.filter(
            Q(player1__user_id__username__iexact=user) & Q(status='accepted') &
...
def GetPlayerByName(name: str, with_no_Friend=False, user=None) -> list:
    if name.count(' ') > 1:
        ...
        data = models.Player.objects.filter(~Q(user_id__pk__in=friend) & (Q(user_id__username__iexact=name) | Q(
